...
config.read(join(PWD, "config.ini"), encoding="utf-8")

# 优化读取逻辑，减少重复
conf = config["WATERMARK"]
水印后缀 = conf.get("水印后缀", "")
水印列数 = conf.getint("水印列数", 3)
水印行数 = conf.getint("水印行数", 3)
水印视图宽度 = conf.getint("水印视图宽度", 300)
...
for name in li:
    ...
    c.rotate(random.randint(295, 335))  # 随机旋转 295-335度, 防止倒置
